#!/usr/bin/env pwsh
# Test de creation de commande avec restaurant_id

Write-Host "TEST CREATION DE COMMANDE AVEC RESTAURANT_ID" -ForegroundColor Cyan
Write-Host ""
...
$commande = @{
    ...
    restaurant_id = 1
    items = @(
        @{id = 1; nom = "Pizza"; prix = 10.50; quantite = 2}
    )
    total = 21.00
} | ConvertTo-Json
